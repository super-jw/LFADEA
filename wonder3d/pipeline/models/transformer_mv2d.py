    ...
    def __call__(
        ...
        if multiview_attention:
            if not sparse_mv_attention:
                ...
                key_front = rearrange(key_raw, "(b t) d c -> b t d c", t=num_views)[:, 0, :, :].repeat_interleave(num_views, dim=0) # [(b t), d, c]
                value_front = rearrange(value_raw, "(b t) d c -> b t d c", t=num_views)[:, 0, :, :].repeat_interleave(num_views, dim=0)
